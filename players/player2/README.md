...
Le 26 janvier, l'antenne AFUP de Rennes organise son premier challenge de codage.
...
Ensuite, on sélectionnera tout ou partie des meilleurs classements et nous procéderons à une analyse de code et de sa performance (les temps de réponse seront mesurés).
...
Le code sera exécuté sur une machine linux avec PHP 8.0 (trop de dépendances non compatibles PHP 8.0 pour le moment) et les extensions de base comme suit:
...
Les batailles entre les codes seront lancées par un agent (voir le script "launch.php" disponible avec ce README).
...
Le code de lancement qui sera utilisé lors de la soirée d'évaluation sera légèrement différent pour lancer tous les codes les uns contre les autres dans des répertoires séparés avec analyse de la performance et des métriques sur les vainqueurs.
...
Quand vous recevez une coordonnée, vous devrez répondre par l'un des résultats suivants :
 ...
 - `cheater [message d'explication]\n` : quand votre concurrent n'a pas envoyé des coordonnées valables

Quand vous recevez un résultat, vous devrez répondre simplement par un `ok\n`. Si la réponse ne vous semble pas normale, vous pouvez répondre à la place `cheater [message d'explication]\n`.

Dans tous les cas, quand l'agent voit passer le message "cheater", la partie est arrêtée et aucun code ne sera considéré gagnant (une analyse des "cheater" sera faite en faisant affronter un code que nous avons développé, et disqualifier ceux qui abusent du "cheater" pour ne pas perdre).  

De même, dans tous les cas, quand un code reçoit un "won", la partie est arrêtée et le code considéré comme gagnant, même si tous les bateaux ne semblent pourtant pas avoir été coulés. Pas la peine d'envoyer un "cheater", ça ne sera pas pris en compte (profitez de votre chance).
